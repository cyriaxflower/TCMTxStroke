...
extern void debug_Say(char *s);		// in drcost.c
extern void cDbg(const char *fmt, ...);	// in drcost.c
extern void hex_dump(void *p, int len);	// in drcost.c
extern int trim(char *s);		// in drcost.c
void  *xmemcpy(void *dst, void *src, int len);	// 舊型手機的 memcpy() 此函數會當機
extern char *xstrcat(char *dst, char* to_add);
...
int f_len, ofs;
char *base_ptr;
int su_main(char *arg)
{	// 傳回 0 代表成功, 傳回 1 代表失敗.
char	*ptr;
FILE	*fhnd;
int	i, len;

ptr = malloc(131072);		base_ptr = ptr;		// debug_Say("in main #52..");	debug_Say(arg);
if (! my_mesg) {
	my_mesg = malloc(65536);
	msg_max = 65536 / sizeof(struct my_message);
	}
fhnd = fopen(arg, "rb");
if (! fhnd) { debug_Say("Open ANN data file failed !\n");  return(1); }		// 傳回 1 代表失敗
fseek(fhnd, 0, 2);
len = ftell(fhnd);
fseek(fhnd, 0, 0);
len = fread(ptr, 1, len, fhnd);
fclose(fhnd);		f_len = len;
// cDbg("read %d bytes from %s OK. 檔案開頭標記 --> %s\n", len, arg, ptr);
load_data(ptr);
debug_Say("將要執行 run_once() !\n");	// 執行單一次神經網路運算 (無回溯, no feedback)
// 清除三層神經細胞為 0
for (i = 0; i < 256; i ++) {
...
// 按照 ANNxxxxxx 來填分數測試 !
// for (i = 0; i < 6; i++) group_score[i] = arg[256 + i];
...
for (i = 0; i < input_count; i ++) cells_input[i] = arg[256 + i];	// 填入輸入層的值

run_once();

debug_Say("神經網路程式順利執行, 返回 !");
arg[258] = (unsigned char) output_count;	// 傳回輸出層的神經細胞數
ofs = 32 + output_count;	// 跳過內部保留訊息長度
for (i = 0; i < output_count; i++) {
	arg[288 + i] = (unsigned char) cells_output[i];	// 填入輸出層的值
	if (cells_output[i] > firing_level) {
		...
		xmemcpy(arg + 256 + ofs, ans, len + 1);
		ofs += len;
		}
	}
ofs = ofs - 32 - output_count;	// 此為真正訊息總長度 (= 2048 - 32 - output_count)
arg[256] = ofs & 255;		arg[257] = (ofs >> 8) & 255;	// 紀錄訊息總長度 (< 2048 字元)
return(0);	// 傳回 0 代表成功
}
...
for (i=0;i < msg->n_lines; i ++) { // 最多取 8 行 (儘量用 Big-5 碼, 儘量勿用 UTF-8, 因為會參雜 < 32 的控制字元)
	if (xstrlen(msg->str[i]) > 0) {
		xstrcat(ans, msg->str[i]);	trim(ans);
		xstrcat(ans, sComma);	// hex_dump(ans, 16);	debug_Say(ans);
		}
	}	// msg->str[i] 的尾端會有一個 0D 00
return(xstrlen(ans));	// 傳回 ans 裡面的文字總長度
}
	...
	if (slen > 0) {	// 有訊息內容
		...
		for (j = 0; j < 8;j ++) { // 最大掃描紀錄 8 行 (成為單一訊息)
			...
			if (crloc > 0) {  // 有找到 chr(13) = CR 換行分割標記
				...
				xmemcpy(msg->str[j], tmp + last_cr + 1, part_len + 1);	// strncpy

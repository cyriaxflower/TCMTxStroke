;!! 訓練檔的證型 TAG 務必擺在第一項  !!  否則無法正確處理 !!  目前不會自動比對證型  TAG !!
...
DataSection
  ; ---- 其他中醫證型 ---- (每一行僅放五個項目, 下面 Data.s 的資料皆同) 此為 RunAnn 顯示給使用者看的檢驗數據名稱, 例如 "風寒" 對應到下面的 "windcold", 餘此類推)
  ...
  ; ---- 其他中醫證型的參數別名 ---- (傳給 AnnCht 的參數, 亦稱 "別名 (alias)", 例如別名 "windcold" 對應到下面的 "CH0001.BIN", 餘此類推)
...
For i = 0  To 99
  ...
  If chdxdat(i) = "end"
    Break                   ; 1 比 1 對應對應 chdxgrp 陣列, 所以不再設置總項數
  EndIf
Next i
...
If nParm > 0
  For i = 0 To nParm - 1
    ...
    If ans$ = "show"
      Continue            ; 忽略此內部隱藏命令, 在本程式的第 70 行已經處裡過 "show" 的命令功能.
    EndIf
    ndx = 9999    ; 100 以內才有效 !
...
If Fid > 0
  toSay$ = "在其他中醫證型, 您選擇: "
  Gosub Say     ; 顯示到除錯訊息文字框
  WriteStringN(Fid, "在其他中醫證型, 您選擇", #PB_Ascii)
  For i_ndx = 0 To work_ndx - 1
    WriteStringN(Fid, Chr(9) + chdxgrp(cht_work(i_ndx)) + " --> ", #PB_Ascii)     ;  + chdxfile(cht_work(i_ndx))
    toSay$ = Chr(9) + chdxgrp(cht_work(i_ndx)) + " --> "    ; + chdxfile(cht_work(i_ndx))
    Gosub Say     ; 顯示到除錯訊息文字框
    ; 呼叫執行所想要的其他中醫證型程式
    InPutNetName$ = curDir$ + chdxfile(cht_work(i_ndx))
    Gosub Load_and_Run
    If Len(Fianl_ans$) > 1
      toSay$ = Fianl_ans$
      Gosub Say     ; 顯示到除錯訊息文字框
      WriteStringN(Fid, Fianl_ans$, #PB_Ascii)
...
If ans_string_count < 1
  WriteStringN(Fid, "抱歉 ! 各證型分數不足, 尚無可建議之方劑", #PB_Ascii)
...
If file_id    ; 0=Err, Not Zero=OK
  ...
  *DATABUF = AllocateMemory(file_len)
  ...
  For i = 0 To 31    ; 每個神經鍵值檔案擁有 32 個訊息結構, 神經鍵值檔案裡面的所有 patterns 皆共用此 32 個訊息結構
    ...
    *ptr = *ptr + 2
    If slen > 0     ; 有訊息內容
      Tmp$ = PeekS(*ptr, slen, #PB_Ascii)  ; 一般視窗電腦 PC 請用 #PB_Ascii    或    蘋果電腦 macOS 請用 #PB_UTF8
      *ptr = *ptr + slen
      ; Debug Tmp$
      For j = 0 To 7

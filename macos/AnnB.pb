...
Global char_width = 11, half_char_width, entropy, entropy_Y, show_entropy = 1, max_run_count = 100
; .b 的數值範圍是   -128 至   +127
...
If LoadFont(1, "細明體", 13)         ; 11 for office, 13 for home.
  can_use_Font_1 = 1
EndIf
  ...
  For run_count = 1 To max_run_count
    ...
    For pattern = 0 To pattern_count - 1
      ...
      ;If training_done(pattern) = 0
        ...
        Debug "#132: 單回合至今 ErrCnt=" + Str(err_count) + " , 第 " + Str(pattern_to_run) +  " 組, 本次樣式 Err count = " + Str(ErrorCount(pattern_to_run)) + " , 本次樣式成功: " + Str(training_done(pattern_to_run)) + " , 熵值: " + Str(entropy) + " , 總執行回合數 = " + Str(run_count)
        ; Gosub Draw_Graphs
      ;EndIf
    Next pattern
    If err_count = 0
...
LoadDataFile:
  If OpenFile(1, "ANNData.BIN", #PB_File_SharedRead)    ; return none-zero = OK
    *DATABUF = AllocateMemory(Lof(1))
    If ReadData(1, *DATABUF, Lof(1)) = 0              ; Failed !
      Return
    EndIf
    *ReadPos = *DATABUF + 128                           ;
    ...
    *ReadPos = *ReadPos + 4
    ...
    For i = 0 To 31
      ...
      *ReadPos = *ReadPos + 2
      For j = 0 To 7
        all_messages.my_message_items(i)\message[j] = PeekS(*ReadPos, 256, #PB_Ascii)    ; !! only ASCII is used now !!
...
FeedBack_Once:
  ...
  For index_fb = 0 To result_count - 1
    ...
    If this_cell <> targets(pattern_to_run, index_fb)   ; (targets 只儲存 0 或 1)
      ...
      If this_cell = 0
        ; Debug "#538 第 " + Str(pattern_to_run) + " 組, 輸出細胞 #" + Str(index_fb) + ": 現值太低 = 0, 目標值是 1 --> add"
        Gosub Add_Net_Down_Power    ; #408 現值太低 = 0, 目標值是 1
      Else
        ; Debug "#541 第 " + Str(pattern_to_run) + " 組, 輸出細胞 #" + Str(index_fb) + " : 現值太高 = 1, 目標值是 0 --> sub"
...
;  OutPutNetName$ = "AN100100.BIN"
  file_id = OpenFile(#PB_Any, OutPutNetName$, #PB_File_SharedWrite)
  ...
  *ptr = *ptr + 8     ; 移位到 48 的位置
  ; 輸出 net_upper(item_count, middle_count) 以 8-bit 無號數儲存 (0 .. 255)
  k = 0     ; 對應輸入層 16 cells, 中間層  32 cells, 輸出層 16 cells <-- 為了 TestSix/AnnCht 等, 需修改..
  For i = 0 To item_count - 1
    For j = 0 To middle_count - 1
...
; IDE Options = PureBasic 6.11 LTS (Windows - x86)
; CursorPosition = 639
; FirstLine = 625
; EnableXP
; DPIAware
; Executable = AnnB0819c.exe
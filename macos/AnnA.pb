...
Global  item_count = 0, middle_count = 0, pattern_count = 0, result_count = 0
...
; 載入學習資料檔
If OpenFile(0, "Data02.txt")
  If ReadFile(0, "Data02.txt")
    Debug FormatDate("Start ! UTC time: %hh:%ii:%ss", DateUTC())
    Format = ReadStringFormat(0)
    If Lof(0) > 0
      While Eof(0) = 0
        StrForParse$ = ReadString(0, Format)
...
Prepare_To_Show:
  ...
  all_messages.my_message_items(return_index)\n_max_chars = max_char_count * 2  ; !!@@ 中文字 @@!!
  For i = 0 To 7
  ...
  *DATABUF = AllocateMemory(131072)   ; 夠放很多資料了 !!
  If *DATABUF
    ...
    *ptr = *DATABUF
    ...
    PokeS(*ptr, info$, 128,  #PB_Ascii)         ; 警告訊息最多放 128 字元
    ...
    *ptr = *ptr + 4
    ...
    For i = 0 To 31
      ...
      *ptr = *ptr + 2
      For j = 0 To 7
        PokeS(*ptr, all_messages.my_message_items(i)\message[j], 256, #PB_Ascii)    ; !! 目前僅寫入 ASCII !!
        *ptr = *ptr + 256
      Next j
    Next i
  EndIf
